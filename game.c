...
#define FOREACH_SURROUNDING_FXY(x, y, a) \
    if (x > 0        && y > 0        && CHECK_MINE(BOARDXY(x - 1, y - 1))) a(x - 1, y - 1); \
    if (                y > 0        && CHECK_MINE(BOARDXY(x    , y - 1))) a(x    , y - 1); \
    if (x < size - 1 && y > 0        && CHECK_MINE(BOARDXY(x + 1, y - 1))) a(x + 1, y - 1); \
    if (x > 0                        && CHECK_MINE(BOARDXY(x - 1, y    ))) a(x - 1, y    ); \
    if (x < size - 1                 && CHECK_MINE(BOARDXY(x + 1, y    ))) a(x + 1, y    ); \
    if (x > 0        && y < size - 1 && CHECK_MINE(BOARDXY(x - 1, y + 1))) a(x - 1, y + 1); \
    if (                y < size - 1 && CHECK_MINE(BOARDXY(x    , y + 1))) a(x    , y + 1); \
    if (x < size - 1 && y < size - 1 && CHECK_MINE(BOARDXY(x + 1, y + 1))) a(x + 1, y + 1);
...
gameInit(int lsize, int lmines) {
    ...
    /* Seed the rand(3) pseudorandom number generator with time(2), good enough entropy */
    srand(time(NULL));
...
gameClearCell(int x, int y) {
    if (CHECK_MINE(BOARDXY(x, y))) {
        ...
        /* Clear clear bit and propagate surrounding cells */
        BOARDXY(x, y) &= ~(1UL << CELL_BIT_CLEAR);
